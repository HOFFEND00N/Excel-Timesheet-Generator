...
test("pass zero non working hours rows, expect to return empty map", () => {
  ...
  expect(actualNonWorkingHoursByEmployees).toEqual(expectedNonWorkingHoursByEmployees);
});
...
test("pass one non working hours row, expect to return map with one element", () => {
  ...
  const actualNonWorkingHoursByEmployees = makeNonWorkingHoursByEmployeesUsername({
    employeeColumn: 4,
    manHoursColumn: 7,
    nonWorkingHoursRows: [[651, "RU", "DaysOff", "Holidays", "Molotkova Maria", "22.01.2021-26.01.2021", "", 24]],
    employees: [...tableData.employees, tableData.teamLead],
  });
...
test("pass two non working hours row with two different employees, expect to return map with two element", () => {
  ...
  const actualNonWorkingHoursByEmployees = makeNonWorkingHoursByEmployeesUsername({
    ...
    manHoursColumn: 7,
    nonWorkingHoursRows: [
      [651, "RU", "DaysOff", "Holidays", "Molotkova Maria", "22.01.2021-26.01.2021", "", 24],
      [651, "RU", "DaysOff", "Sick1DQ", "Matrosova Marianna", "23.01.2021", "", 8],
    ],
    employees: [...tableData.employees, tableData.teamLead],
  });
...
test("pass non working hours row with non-existent employee, expect to skip this row", () => {
  ...
  const actualNonWorkingHoursByEmployees = makeNonWorkingHoursByEmployeesUsername({
    ...
    nonWorkingHoursRows: [
      [651, "RU", "DaysOff", "Sick1DQ", "Molotkova Maria", "27.01.2021", "", 8],
      [651, "RU", "DaysOff", "Sick1DQ", "Koroleva Glasha", "17.01.2021", "", 8],
    ],
    employees: [...tableData.employees, tableData.teamLead],
  });
